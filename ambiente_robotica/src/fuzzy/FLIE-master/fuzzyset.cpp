...
void trapezoid_category::setname(const char *n)
{
strcpy(name,n);
}
...
// Special Case: Fuzzy Set QZ(Quasi Zero), that is simetric, centered on zero!!!(JAF 13/12/2001)
     if(!strcmp(name, "QZ"))
      ...
      highval = randomnum(0,rangehigh);
      lowval = -highval;
      }
     else
      {
      lowval = rangelow + randomnum(0,rangehigh - rangelow);
      highval = rangehigh - randomnum(0,rangehigh - lowval);
      midvallow=midvalhigh = lowval + randomnum(0,highval - lowval);
      }
}
...
//                      midval between lowval(midvallow) and highval
     ...
     highval = rangehigh - randomnum(0,rangehigh - lowval);
     midvalhigh = lowval + randomnum(0,highval - lowval);
}
...
//                      midvallow between lowval and highval
     ...
     lowval = rangelow + randomnum(0,highval - rangelow);
     midvallow = lowval + randomnum(0,highval - lowval);
...
{
  float newlowval;
  newlowval = lowval + (randomnum(rangelow,rangehigh)*0.1);// Variation: +-10% of range of category
...
{
  float newmidvallow;
  newmidvallow = midvallow + (randomnum(rangelow,rangehigh)*0.1);// Variation range: -7.5~+7.5
...
{
  float newmidvalhigh;
  newmidvalhigh = midvalhigh + (randomnum(rangelow,rangehigh)*0.1);// Variation range: -7.5~+7.5
...
{
  float newhighval;
  newhighval = highval + (randomnum(rangelow,rangehigh)*0.1);// Variation range: -7.5~+7.5
...
float randomnum(float minval, float maxval)
{
